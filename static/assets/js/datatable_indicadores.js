...
var KTDatatablesExample = function () {
    ...
    var initDatatable = function () {
        ...
        datatable = $(table).DataTable({
                ...
                //             if (last !== group) {
                //                 $(rows).eq(i).before(
                //                         '<tr class="text-center fs-5 text-gray-900 fw-bolder border border-2 border-success"><td colspan="4">' +
                //                         group +
                //                         '</td></tr>'
                //                     );
                //
...
// Hook export buttons
    var exportButtons = () => {
        const documentTitle = 'Reporte de indicadores de acciones de adaptación para el cambio climático.';
...
// Search Datatable --- official docs reference: https://datatables.net/reference/api/search()
    ...
    var handleDeleteAll = () => {
        ...
        deleteAllBtn.addEventListener('click', function () {
            ...
            Swal.fire({
                text: `¿Está seguro que desea eliminar ${rowsToDelete.length} indicador(es) seleccionado(s)?`,
                icon: "warning",
                ...
                if (result.value) {
                    ...
                    rowsToDelete.forEach(row => {
                        const idIndicador = parseInt(row.getAttribute('data-id'));
                        const idAcion = parseInt(deleteAllBtn.getAttribute('data-accion-id'));
                        fetch(`/accion/${idAcion}/indicadores/eliminar/${idIndicador}/`, {
                            method: 'POST',
                            headers: {
...
// Public methods
    return {
        init: function () {
            table = document.querySelector('#kt_datatable_indicadores_adaptacion');
